...
module.exports = {
  ...
  execute: async (client, message, args, prefix, guild, color, channel) => {
    ...
    //SHOPBUY 2
    if (args[0] =="2"){
      if(!args[1]) return message.channel.send("Invaild PID")
      if(args[2].toLowerCase()=="stone") return message.channel.send("INVAILD PID")
    }

    //SHOPBUY 3
    if (args[0] =="3"){
      return message.channel.send("PID Invaild!")
    }
    ...
    if (args[0] == "5") {
      if(!args[1]) return message.channel.send("Please specify the Item you want to Buy!")
        ...
        if (user.balance < 1000) return message.channel.send('You must need 1000 cc to mega your Pokemon.');
        else {
    ...
    if (args[0].toLowerCase() == "6") {
      ...
      if (args[1].toLowerCase() == '3sh') {
        if (user1.balance < 5000) return message.channel.send("You don't have enough Balance (5000) to Exchange for 3 Shards.");
        let embed = new MessageEmbed()
        .setAuthor("Shop Exchange Shards")
        .setDescription(`${message.author.username}\n\n**Your Shards**: ${user1.shards}\n**Your Balance**: ${user1.balance}\n\nPlease confirm if you would like to buy 3 Shards for 5000 . React with ✅ to Confirm or ❌ to Cancel.`)
        .setColor(color)
        let msg = await message.channel.send(embed);
        await msg.react("✅");
        msg.react("❌");
      ...
      if (args[1].toLowerCase() == '10sh') {
        if (user1.balance < 10000) return message.channel.send("You don't have enough Balance (10000)to buy 10 Shards.");
        let embed = new MessageEmbed()
        .setAuthor("Shop Exchange Shards")
      ...
      if (args[1].toLowerCase() == '25sh') {
        if (user1.balance < 20000) return message.channel.send("You don't have enough Balance (20000) to buy 25 Shards.");
        let embed = new MessageEmbed()
        .setAuthor("Shop Exchange Shards")
        .setDescription(`${message.author.username}\n\n**Your Shards**: ${user1.shards}\n**Your Balance**: ${user1.balance}\n\nPlease confirm if you would like to buy 25 Shards for 20000. React with ✅ to Confirm or ❌ to Cancel.`)
        .setColor(0x00f9ff)
        let msg = await message.channel.send(embed);
        await msg.react("✅");
        msg.react("❌");
      ...
      if (args[1].toLowerCase() == '80sh') {
        if (user1.balance <= 50000) return message.channel.send("You don't have enough Balance (50000) to buy 80 Shards.");
        let embed = new MessageEmbed()
        .setAuthor("Shop Exchange Shards")
        .setDescription(`${message.author.username}\n\n**Your Shards**: ${user1.shards}\n**Your Balance**: ${user1.balance}\n\nPlease confirm if you would like to buy 80 Shards for 50000 . React with ✅ to Confirm or ❌ to Cancel.`)
        .setColor(0x00f9ff)
        let msg = await message.channel.send(embed);
        await msg.react("✅");
        msg.react("❌");
      ...
      if (args[1].toLowerCase() == '200sh') {        
        if (user1.balance <= 100000) return message.channel.send("You don't have enough Balance (100000) to buy 200 Shards.");
        let embed = new MessageEmbed()
        .setAuthor("Shop Exchange Shards")
    ...
    if (args[0] == "10") {
      if(!args[1]) return message.channel.send("Please specify the Item you want to Buy!")
        ...
        if (user.balance < 25000) return message.channel.send('You must need 25000 ツ to Gmax your Pokemon.');
        else {
    ...
    if (args[0] == "11") {
      if(!args[1]) return message.channel.send("Please specify the Item you want to Buy!")
      if (args[1].toLowerCase()==="form"){
        if (args[2].toLowerCase() == "shadow" || args[2].toLowerCase() == "shad") {
        let user = await User.findOne({ id: message.author.id });
        if (!user || !user.pokemons[0]) return message.channel.send("You need to pick a starter pokémon using the \`" + prefix + "start\` command before using this command!");
        var selected = user.selected || 0;
        var name = user.pokemons[selected].name;
        ...
        if (user.balance < 10000) return message.channel.send('You must need 10000 ツ to Shadow Transform your Pokemon.');
        else {
