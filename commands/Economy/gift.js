...
module.exports = {
  ...
  execute: async (client, message, args, prefix, guild, color, channel) => {
    ...
    .setDescription("Gift your Items to another Trainer.")
    ...
    if (args[0].toLowerCase()=="redeem"||args[0].toLowerCase()=="r"){
      ...
      if (args[0].toLowerCase()=="cc"||args[0].toLowerCase()=="balance"||args[0].toLowerCase()=="bal"){
      if (user.balance == 0 ) return message.channel.send("You don't have any Balance to Gift!")
      if (user1.id == message.author.id) return message.channel.send(`Strange! You wanna Gift your own Items to Yourself??`);
      if (amount==1 && amount>user.balance) return message.channel.send(`You don't have ${amount}  to Gift!`)
      if (amount>=2 && amount>user.balance) return message.channel.send(`You don't have ${amount}  to Gift!`)
      ...
      if(amount==1){
      msg = await message.channel.send(`Do you Confirm to gift ${amount}  to ${user1.user.username}?`);
      }
      else if (amount>1){
      msg = await message.channel.send(`Do you Confirm to gift ${amount} to ${user1.user.username}?`);
      }
