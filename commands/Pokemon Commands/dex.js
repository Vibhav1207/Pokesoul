        ...
        if (!args[0]) {
            ...
            for (let a = 0; a < txt.length && a < 18; a++) {
                if (user.caught.find(r => r.name.replace(/-+/g, "-").toLowerCase() == txt[a].replace(/-+/g, "-").toLowerCase())) {
                    embeddex
                        .addField(`${txt[a]}`, ' <:pokeball_flat:886503867469168650> **Caught**', true)
                        .setDescription(`**__Dex Count__**: ${dex.length}\n**__Pokemon's Caught**__**: ${n}\n ════════════════════════════════════\n`);
                    await true;
                } else {
                    embeddex
                        .addField(`${txt[a]}`, ' <:pokeball_empty:886504636134068295> **Not caught yet** ', true)
                        .setDescription(`**__Dex Count__**: ${dex.length}\n**__Dex Entries__**: ${n}\n ═══════════════════════════════════════ \n`);
                }
            }
            ...
            if (args.length === 1 && Number(args[0])) {
                ...
                for (let a = msw; a < txt.length && a < msw + 18; a++) {
                    if (user.caught.find(r => r.name.replace(/-+/g, "-").toLowerCase() == txt[a].replace(/-+/g, "-").toLowerCase())) {
                        ...
                        embeddex
                            .addField(`${txt[a]}`, ' :x: Not caught yet', true)
                            .setDescription(`**Dex Count**: ${dex.length}\n**Dex Entries**: ${n}`);
                    }
                }
                embeddex
